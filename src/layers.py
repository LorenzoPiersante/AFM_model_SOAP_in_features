...
class attention_mech(nn.Module):
    '''
    Attention mechanism for node features output of MPNN.
    The function assumes a 3D tensor input.
    ...
    Forward args:
    - tensorL: tensor input for keys, dim(tot_num_nodes, num_env, input_r).
    - tensorH: tensor input for queries, dim(tot_num_nodes, num_env, input_c).
    Where tot_num_nodes is the total  number of nodes in the batch and num_env is the total
    number of chemical environments in the system.

    Returns:
    - outputs: dim(tot_num_nodes, num_env, output_s), environment features for each node subject
    to attention, ideally the most relevant feature will be emphasized over the different chem
    environments for each node.
    '''

    def __init__(self, input_r, input_c, output_s, device):
        super().__init__()
        self.device = device

        self.keys = nn.Sequential(nn.Linear(input_r, output_s), nn.Tanh(),
                                  nn.Linear(output_s, output_s))

        self.queries = nn.Sequential(nn.Linear(input_c, output_s), nn.Tanh(),
                                     nn.Linear(output_s, output_s))

        self.values = nn.Sequential(nn.Linear(2 * output_s, output_s), nn.Tanh(),
                                    nn.Linear(output_s, output_s))
    ...
    def forward(self, tensorL, tensorH):
        ...
        # apply attention
        attention = torch.matmul(keys, torch.transpose(queries, 1, 2))
        ...
        value_inputs_list = []
        for n in range(keys.size(1)):
            # allocate memory
            new_tensor = torch.zeros((queries.size(0), queries.size(1), 2 * queries.size(2)),
                                     requires_grad=False, dtype=torch.float, device=self.device)
            # assign all queries to input
            new_tensor[:, :, 0:queries.size(2)] = queries
            # assign specific key to input
            new_tensor[:, :, queries.size(2):2 * queries.size(2)] = torch.unsqueeze(keys[:, n, :], dim=1)
            # append to input list
            value_inputs_list.append(new_tensor)
        # stack along dim=1, new keys dimension
        value_inputs = torch.stack(value_inputs_list, dim=1)
        # resulting tensor is dim(num_nodes, num_keys, num_queries, value_size)
        ...
        outputs = torch.squeeze(outputs, dim=2)  # remove dummy dimension
